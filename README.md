...
The app starts the engine for you: on launch it checks the API, and if nothing answers it spawns the engine as a supervised child (restarted on crash, stopped when the app quits — the engine watches its stdin pipe, so it exits even if the app is force-killed). It looks for the binary in the app bundle first, then falls back to `engine/target/{release,debug}/` for development, or `NETSENTRIX_ENGINE_BIN`. Manual `cargo run` still works; the app detects an already-running engine and leaves it alone. Auto-start can be turned off in Settings (⌘,) → Engine.
...
```bash
make bundle          # dist/NetSentrix.app — release app + embedded engine, icon, ad-hoc signed
make bundle-app-only # skip building/embedding the engine
```
